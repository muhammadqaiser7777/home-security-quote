...
export class Form implements OnInit {
  ...
  validateCurrentStep(): boolean {
    let valid = true;
    if (this.currentStep === 1) {
      if (!this.HomeSecurity) {
        this.errors['HomeSecurity'] = 'Please select a project type.';
        valid = false;
      }
  ...
  async submit() {
    ...
    if (this.validateCurrentStep()) {
      ...
      const payload = {
        HomeSecurity: parseInt(this.HomeSecurity),
      ...
      this.http.post('https://home-security-quote.com/api/ping-proxy.php', payload).subscribe({
        next: (response) => {
